...
class TimeseriesDataSource(CsvDataSource):
    ...
    def _subtract(self, values0, values1):
        return [value0 - value1 for value0, value1 in zip(values0, values1)]

    def _subtract_values_from_series(self, series, values):
        return [self._subtract(row, values) for row in series]
    ...
    def _load_timeseries(self):
        for i in range(self._original_size):
            ...
            for j in range(0,len(series[0])-self._x_input_length-self._x_output_length,self._x_split_step):
                xin = self._subtract_values_from_series(
                            series[0][j:j+self._x_input_length],
                            series[0][j+self._x_input_length-1])
                xout = self._subtract_values_from_series(
                            series[0][j+self._x_input_length:j+self._x_input_length+self._x_output_length],
                            series[0][j+self._x_input_length-1])
                self._xdata_in.append(xin)
                self._xdata_out.append(xout)
