...
class Network(chainer.Chain):
    ...
    def __init__(self, n_out):
        ...
        with self.init_scope():
            self.c1 = L.Convolution2D(None, out_channels=16, ksize=(7, 7), stride=(1, 1), pad=(0, 0), nobias=True)
            self.b1 = L.BatchNormalization(16)
            self.c2 = L.Convolution2D(None, out_channels=30, ksize=(3, 3), stride=(1, 1), pad=(0, 0), nobias=True)
            self.b2 = L.BatchNormalization(30)
            self.l1 = L.Linear(None, 160)
            self.b3 = L.BatchNormalization(160)
            self.l2 = L.Linear(None, n_out)

    def __call__(self, x):
        h1 = F.relu(self.c1(x))
        h2 = self.b1(h1)
        h3 = F.max_pooling_2d(h2, ksize=(2, 2), stride=(2, 2), pad=(0, 0))
        h4 = F.relu(self.c2(h3))
        h5 = self.b2(h4)
        h6 = F.max_pooling_2d(h5, ksize=(2, 2), stride=(2, 2), pad=(0, 0))
        h7 = F.relu(self.l1(h6))
        h8 = self.b3(h7)
        return F.softmax(self.l2(h8))
...
class LeNet:
    ...
    def infer(self, x):
        ...
        x = np.float32(x.transpose(0, 3, 1, 2))
        x = self._model.xp.asarray(x)
...
def get_args(model_params_path='model.npz', training_dataset_path="trining.csv",
    ...
    parser.add_argument('--out', '-o', default='chainer_lenet_result',
...
if __name__ == '__main__':
    ...
    if config.process == 'train':
        ...
        for i in range(len(test_dataset)):
            x = test_dataset[i]
            result = model.infer(x[0].transpose(1, 2, 0))
