...
from tensorflow.python.keras.layers import Dense, BatchNormalization, Flatten
...
class LeNet:
    ...
    def create_model(self, x, y):
        return Sequential([
            Conv2D(filters=16, kernel_size=(7,7), strides=(1,1), padding='valid',
                    activation='relu', input_shape=(x.shape[1], x.shape[2], 1)),
            BatchNormalization(),
            MaxPooling2D(pool_size=(2,2),strides=(2,2)),
            Conv2D(filters=30, kernel_size=(3,3), strides=(1,1), padding='valid',
                    activation='relu'),
            BatchNormalization(),
            MaxPooling2D(pool_size=(2,2),strides=(2,2)),
            Flatten(),
    ...
    def infer(self, series):
        if len(series.shape) == 2:
            series = series.reshape(1, series.shape[0], series.shape[1], 1)
        elif len(series.shape) == 3:
            series = series.reshape(1, series.shape[0], series.shape[1], series.shape[2])
