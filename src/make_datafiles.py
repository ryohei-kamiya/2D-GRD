...
class DatasetMaker:
    ...
    def _standardize(self, points):
        result = []
        for point in points:
            result.append([point[0] / self._max_scale - 1.0, point[1] / self._max_scale - 1.0])
        return result

    def _unstandardize(self, points):
        result = []
        for point in points:
            result.append([(point[0] + 1.0) * self._max_scale, (point[1] + 1.0) * self._max_scale])
        return result

    def run(self, input_dir, output_dir):
        points_dir = os.path.join(output_dir, 'points')
        image_dir = os.path.join(output_dir, 'image')
        sampled_points_dir = os.path.join(output_dir, 'sampled_points')
        make_dir(points_dir)
        make_dir(image_dir)
        make_dir(sampled_points_dir)
        for dname in os.listdir(input_dir):
            ...
            if os.path.isdir(i_dname):
                ...
                o_sampled_points_dname = os.path.join(sampled_points_dir, dname)
                make_dir(o_points_dname)
                make_dir(o_image_dname)
                make_dir(o_sampled_points_dname)
                ...
                for i_file in i_files:
                    ...
                    for points in points_list:
                        self._points_buf.set_points(points)
                        self._points_buf.adjust_scale()
                        points = self._points_buf.get_points()
                        points = self._unstandardize(points)
                        points = self._fulfill_interpolation(points)
                        ...
                        self._save_points(points, o_points_fname)
                        self._save_image(points, o_image_fname)
                        o_sampled_points_fname = os.path.join(o_sampled_points_dname, "{0}-{1:04d}.csv".format(fname, fnum))
                        self._points_buf.set_points(points)
                        self._points_buf.adjust()
                        sampled_points = self._points_buf.get_points()
                        self._save_points(sampled_points, o_sampled_points_fname)
                        fnum += 1
